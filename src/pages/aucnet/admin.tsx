...
import { sortModes, auctionGrade, model, customer, region } from "../../components/AucNetComponents/Datas/generateData";
...
const AdminPage: React.FC = () => {
  ...
  const handleBulletClick = (bulletTitle: string) => setActiveBullet(bulletTitle);
  ...
  const handleRegionChange = (selectedItems: string[]) => setSelectedRegion(selectedItems);
  const handleCustomerChange = (selectedItems: string[]) => setSelectedCustomer(selectedItems);
  const handleModelChange = (selectedItems: string[]) => setSelectedModel(selectedItems);
  const handleGradeChange = (selectedItems: string[]) => setSelectedGrade(selectedItems);
  ...
  const CarList = () => {
    const filteredCars = cards.filter((car: any) => {
      return (
        ...
        (selectedCustomer.length === 0 || selectedCustomer.includes(car.customer)) &&
      ...
      <tbody>
        {sortedCars.map((car: any, index: number) => (
          <AdminTableRow
            ...
            onCheckboxChange={(checked:any) => handleCheckboxChange(index, checked)}
          />
        ))}
      </tbody>
    );
    ...
    <div className="w-full h-fit min-h-screen px-8 md:px-16 lg:px-32 py-24 bg-slate-50">
      ...
      <div className="flex flex-col gap-2 pb-4 border-b border-b-gray-200">
        <div className="flex gap-2">
          {[
            { title: "New", color: "bg-yellow-200", count: cards.filter((car:any) => car.status === 'New').length },
            { title: "Approved", color: "bg-amber-200", count: cards.filter((car:any) => car.status === 'Approved').length },
            { title: "Qualified", color: "bg-orange-300", count: cards.filter((car:any) => car.status === 'Qualified').length },
            { title: "Processed", color: "bg-green-300", count: cards.filter((car:any) => car.status === 'Processed').length },
            { title: "Finished", color: "bg-blue-200", count: cards.filter((car:any) => car.status === 'Finished').length },
      ...
      <ErrorBoundary>
        <Suspense fallback={<div className="flex items-center gap-2 py-2"><TbLoader2 className="animate-spin"/>Fetching Cars...</div>}>
